...
const { Product, Vendor, User, item } = require("../models");
...
const productController = {
  ...
  getAllProducts: async (req, res) => {
    try {
      ...
      const items = await item.findAll({ where: { vendorId: vendor.id } });
      if (items === 0) {
        return res.status(404).send("Products Not Found at the moment");
      }
