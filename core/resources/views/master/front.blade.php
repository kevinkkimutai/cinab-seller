...
<style>
    .gifAnimators{
        height: 60px;
        background-image: url({{asset("/assets/images/defaultproductimage/cinablogo.gif")}});
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="d-flex justify-content-between">
                    ...
                    @endif
                    <div class="toolbar-item visible-on-mobile"><a href="{{route('front.cart')}}">
                    ...
                    <div class="login-register text-black toolbar-item visible-on-mobile ">
                        @if(!Auth::user())
                        <a class="track-order-link ml-md-2 text-black text-center pt-md-4 pt-2 text-md font-md" href="{{route('user.login')}}">
                        {{__('Login')}}
                        </a>
                        @else
                        <div class="t-h-dropdown">
...
<div class="row w-100">
    <ul class="navbar-nav justify-content-center d-flex pl-10">
    <li class="nav-item">
    @if ($setting->is_shop == 1)
       <a class="{{ request()->routeIs('front.catalog*') }} nav-link" href="{{route('front.catalog')}}">{{__('Shop')}}</a>
    @endif
    </li>
    @foreach ($categories as $key => $pcategory)
            <li class="nav-item dropdown">
                <a class="nav-link" href="{{ route('front.catalog').'?category='.$pcategory->slug }}" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    {{ $pcategory->name }}
                </a>
                @if ($pcategory->subcategory->count() > 0)
                    <ul class="dropdown-menu p-1 " aria-labelledby="navbarDropdown">
                        @foreach ($pcategory->subcategory as $scategory)
                            <li><a class="font-semibold text-black">{{ $scategory->name }}</a></li>
                            @if ($scategory->childcategory->count() > 0)
                            <li class="child-category">

                              @foreach ($scategory->childcategory as $childcategory)
                              <a class="dropdown-item" href="{{route('front.catalog').'?childcategory='.$childcategory->slug}}">{{$childcategory->name}}</a>
                              @endforeach
                            </li>
                            @endif
                        @endforeach
        ...
        @endforeach
         <li class="nav-item">
        @if ($setting->is_blog == 1)
<a class="{{ request()->routeIs('front.blog*') }} nav-link" href="{{route('front.blog')}}">{{__('Construction')}}</a>
@endif            
        </li>
    </ul>
    </div>
 ...
 <footer class="site-footer">
    <div class="container">
        ...
        <div class="col-lg-4 col-md-6">
          ...
          @endphp
            <div class="footer-social-links">
                ...
                <a target="_blank" href="{{$link}}"><span><i class="{{$icons[$link_key]}}"></i></span></a>
                @endforeach
            </div>
          </section>
        </div>
                              ...
                              <table class="table table-responsive table-borderless">
                                 <thead>
                                  <th colspan="2" class="text-uppercase">
                                      DOWNLOAD CINAB FREE APP
                                  </th>
                                 <th></th>
                                  </thead>
                                  <tbody>
                                     <tr>
                                         <td class="text-right"> <a  target="_blank" href="https://apps.apple.com/ke/app/cinabonline/id1549029666"><img src="{{asset("assets/downloadimages/apple.png")}}" height="30"> </a></td>
                                         <td class="text-left"> <a  target="_blank" href="https://play.google.com/store/apps/details?id=com.CINAB"><img src="{{asset("assets/downloadimages/google.png")}}" height="30"> </a></td>
                                     </tr>
                                  </tbody>
                              </table>
                          </div>
      ...
      <p class="footer-copyright"> {{$setting->copy_right}}</p>
        @include("z_analysis.getuserlocation")
    </div>
  </footer>
