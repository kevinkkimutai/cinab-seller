...
use App\{Http\Controllers\Front\MpesaPaymentController,
    Models\Order,
    Models\Setting,
    Models\TrackOrder,
    Helpers\EmailHelper,
...
trait MpesaPayment
{
    public function mpesaPayment($data)
    {
        //return $data;
        ...
        //TODO MPESA payment to start here
         $phoneNo = "254" . substr($data['phoneNo'], -9);
        //$phoneNo = "254" . substr("0721729081", -9);
        $amount = round(str_replace(',', '', $total_amount),0);
        //$amount = str_replace(',', '', "20000");


        if (MpesaPaymentController::MakeSTKPushPayment($phoneNo, $amount, $userID, $transaction_number) == true) {
            ...
            return [
                'status' => true,
                'message' => "We have received your request and we are working on it. You will receive a prompt to make payment shortly."
            ];
        } else {
            return [
                'status' => true,
                'message' => "Error processing your request. Try again after a few minutes"
            ];
        }
    }
}